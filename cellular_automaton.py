    ...
    for step in range(time_steps):
        ...
        if (step % (sqrt2 + 1)) < 1:
            for row in range(rows_total):
                for column in range(columns_total):
                    if lattice_frozen[row, column] != config.cell_blocked:
                        lattice[row, column] = cell_update_moore(
                            row,
                            column,
                            rows_total,
                            columns_total,
            ...
            for row in range(rows_total):
                for column in range(columns_total):
                    if lattice_frozen[row, column] != config.cell_blocked:
                        lattice[row, column] = cell_update_von_neumann(
                            row,
                            column,
                            rows_total,
                            columns_total,
...
def cell_update_moore(
    row, column, rows_total, columns_total, neighbors, lattice_frozen
):
        ...
        for delta_column in neighbors:
            ...
            if (current_neighbor[0] >= 0) and (current_neighbor[0] < rows_total):
                if (current_neighbor[1] >= 0) and (current_neighbor[1] < columns_total):
                    if (
                        lattice_frozen[current_neighbor[0], current_neighbor[1]]
                        > current_cell
                    ):
                        propagation_flag = 1
